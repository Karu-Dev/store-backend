      ...
      if (server.name === serverName) {
        ...
        if (!ItemsController.activeServers[serverName]) {
          ...
          if (
            ...
            ItemsController.errorFrequency
          ) {
            ItemsController.activeServers[serverName];
            if (ItemsController.activeServers[serverName] === 0) {
              ItemsController.activeServers[serverName] = 10;
            }
            return false;
          } else {
            ItemsController.activeServers[serverName]--;
      ...
      if (Object.keys(req.body).length === 1 && req.body.servers.length > 0) {
        for (let x in req.body.servers) {
          if (Object.keys(req.body.servers[x]).length === 2) {
            if (
              typeof req.body.servers[x].name === "string" &&
              req.body.servers[x].name.length > 0
            ) {
              if (
                typeof req.body.servers[x].errorFrequency === "number" &&
                req.body.servers[x].errorFrequency >= 0 &&
                req.body.servers[x].errorFrequency <= 10
              ) {
                continue;
              } else {
                res.sendStatus(400);
